...
app.get("/api/leetcode/:username", async (req,res)=>{
  ...
  try {
    ...
    const normalized = {
      ...
      contributionsCalendar: data?.contributionsCalendar || {}, // if API provides
      submissions: (data?.recentSubmissionTimes || []).map(ts => ({ timestamp: ts }))
    };
    res.json(normalized);
  } catch(err){
    ...
    res.json({
      ...
      contributionsCalendar: {}, submissions: []
    });
  }
});

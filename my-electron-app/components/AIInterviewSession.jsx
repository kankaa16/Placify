  ...
  try {
    if (!isMicRecording) {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        if (data.transcript) {
          // send this transcript to AI
          handleSubmitTranscriptFromMic(data.transcript);
        }
      };
...
const handleSubmitTranscriptFromMic = async (spokenText) => {
  const userMessage = { role: "user", content: spokenText };
  const newHistory = [...conversationHistory, userMessage];
  setConversationHistory(newHistory);
  setTranscript(spokenText); // optional for display
  ...
  <div className="score-modal-overlay">
    <div className="score-modal">
      ...
      <p>Technical: {showScore.technicalScore}</p>
      <p>Communication: {showScore.communicationScore}</p>
      <p>Confidence: {showScore.confidenceScore}</p>
      <h3>Overall: {showScore.overallScore}</h3>
      <p>{showScore.feedback}</p>

      <button onClick={() => setShowScore(null)}>Close</button>
    </div>
  </div>
)}

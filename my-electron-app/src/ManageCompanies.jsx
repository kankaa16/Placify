...
import { Building2, Trash2, Edit2, Plus, MapPin, Calendar, Link2 } from "lucide-react";
...
const ManageCompanies = () => {
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:5000/api/companies/${editing}`, {
        method: "PUT",
  ...
  return (
    <div className="manage-container">
      <div className="header-bar">
        <h1>
          <Building2 className="icon" /> Manage Companies
        </h1>
        <div className="btns">
            ...
            >
              {editing === company._id ? (
                <form onSubmit={handleEditSubmit} className="edit-form">
                  ...
                  <input name="role" value={form.role} onChange={handleEditChange} />
                  <input name="ctc" value={form.ctc} onChange={handleEditChange} />
                  <input name="criteria" value={form.criteria} onChange={handleEditChange} />
                  <textarea
                    name="description"
                    rows="2"
                  ...
                  />
                  <input name="location" value={form.location} onChange={handleEditChange} />
                  <input name="lastDate" value={form.lastDate} onChange={handleEditChange} />
                  <input name="applyLink" value={form.applyLink} onChange={handleEditChange} />
                  <button type="submit" className="save-btn">Save</button>
                  <button onClick={() => setEditing(null)} type="button" className="cancel-btn">Cancel</button>
                </form>
              ) : (
                <>
  ...
  <div className="cover-overlay">
    ...
    <h3 className="company-role">{company.role}</h3>
    {company.location && (
                  ...
                  <div className="card-body">
                    <div className="card-header">
                      ...
                      <div className="actions">
                        <Edit2 onClick={() => startEdit(company)} className="edit-icon" />
                        <Trash2 onClick={() => handleDelete(company._id)} className="delete-icon" />
                      </div>
                    </div>

<div className="cover-details">
      {company.ctc && (
        <p className="cover-ctc">
          <strong>CTC:</strong> INR {company.ctc}
        </p>
      )}
      {company.criteria && (
        <p className="cover-criteria">
          <strong>Eligibility:</strong> {company.criteria}
        </p>
      )}
    </div>
